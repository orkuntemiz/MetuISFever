...
full_abs_list,ranked_aswers,pandasData=displayResults(hit_dictionary, answers, query)

def searchDatabase(question, keywords, pysearch, lucene_database, minDate='2019/12/01', k=25):
    ## search the lucene database with a combination of the question and the keywords

    searcher = pysearch.SimpleSearcher(luceneDir)
    hits = searcher.search(query_processed + '. ' + keywords,k=k)
    n_hits = len(hits)
    ## collect the relevant data in a hit dictionary
    hit_dictionary = {}
    for i in range(0, n_hits):
    ...
    full_abs_list,ranked_aswers,pandasData=displayResults(hit_dictionary, answers, question)
    return full_abs_list,ranked_aswers, pandasData
...
minDate='2019/12/01'
if 1==1:
  full_abs_list,ranked_aswers,pandasData=searchDatabase(query, keywords, pysearch, luceneDir, minDate=minDate, k=10)
  probabilities,verdict=CheckEntailmentNeturalorContradict(ranked_aswers,query,pandasData)